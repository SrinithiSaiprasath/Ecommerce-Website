...
router.post('/add', async (req, res) => {
  try {
    ...
    await mailchimp.sendMerchantApplicationEmail(email);
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await mailchimp.sendMerchantDeactivateEmail(merchantDoc.email);
    }
...
const deactivateBrand = async merchantId => {
  ...
  if (!merchantDoc || !merchantDoc.brand) {
    return;
  }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await mailchimp.sendMerchantWelcomeEmail(email, name);
    ...
    await mailchimp.sendMerchantSignupEmail(email, host, {
      resetToken,
      email
    });
